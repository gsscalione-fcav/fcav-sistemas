...
--*    
--* ALTERAÇÕES:     
--*  17/09/2018: Adicionado a validação para trazer o valor da RPS quando o desconto for na dívida.  Cancelado
--* Autor:    
--*     
--* ***************************************************************    
...
--BEGIN  
--  SELECT @p_valorServicoRPS = sum(VALOR)
--  FROM LY_ITEM_LANC   
--  WHERE BOLETO = @p_boleto  
--  AND ITEMCOBRANCA = 1  
--  group by BOLETO
...
-- Verificação para não gerar o RPS quando for teste  
  ...
  FROM VW_COBRANCA_BOLETO B 
   JOIN LY_COBRANCA C ON B.COBRANCA = C.COBRANCA    
   JOIN LY_ALUNO A ON C.ALUNO = A.ALUNO    
   JOIN LY_RESP_FINAN R ON C.RESP = R.RESP    
  WHERE B.BOLETO = @p_boleto  
  AND R.CGC_TITULAR NOT LIKE '26749984000100'			-- PARA NÃO PEGAR A ÚNICA EMPRESA VALIDA QUE POSSUI TESTE NO NOME. GABRIEL
