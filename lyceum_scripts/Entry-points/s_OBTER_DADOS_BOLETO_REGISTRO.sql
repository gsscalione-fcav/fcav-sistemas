/*  
  SP s_OBTER_DADOS_BOLETO_REGISTRO  
  
Description:  Obtem os dados dos boletos para registro.  
    
Custom:   
 18/02/2019 - LIMITE O NOME DO PAGADOR PARA ATÉ 40 CARACTERES. Gabriel S.S.  
  
Author: Techne  
Date: 2018-11-15 00:30:08.490  
*/  
...
CREATE PROCEDURE s_OBTER_DADOS_BOLETO_REGISTRO    
 ...
 --LIMITA O NOME DO PAGADOR PARA ATÉ 40 CARACTERES  
 DECLARE @nome_pagador VARCHAR(100)  
 DECLARE @resp T_CODIGO  
  
 SELECT @nome_pagador = TITULAR  
 FROM LY_RESP_FINAN  
 WHERE RESP = (SELECT RESP FROM LY_BOLETO WHERE BOLETO = @p_boleto)  
  
   SET @p_nome_pagador = LEFT(@nome_pagador, 40)  
 -----------------------------------------------------------------  
