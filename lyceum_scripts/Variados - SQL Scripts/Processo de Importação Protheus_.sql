...
SET @DATA_INI = '2017-08-01'
SET @DATA_FIM = '2017-10-04'  
...
-- '1 - FAT',
 ...
 '01' AS MOEDA,  
 '3' AS TIPO_LANC,  
 CASE WHEN ILAN.MOTIVO_DESCONTO = 'ESTORNO' THEN '31401002'  
   WHEN ILAN.DESCRICAO LIKE 'DESCONTO%' THEN '31401001'  
   WHEN ILAN.DESCRICAO LIKE 'Estorno de Cobrança: Estorno de Bolsa%' THEN '31401003'  
   WHEN ILAN.DESCRICAO LIKE 'Estorno de Cobrança: Estorno de Desconto%' THEN '31401001'  
   WHEN ILAN.DESCRICAO LIKE 'ESTORNO%' THEN '31401003'  
   WHEN ILAN.DESCRICAO LIKE 'BOLSA%' THEN '31401003'  
   ELSE '11204001' END AS CONTA_DEB,  
 CASE WHEN ILAN.MOTIVO_DESCONTO = 'ESTORNO' THEN '11204001'  
   ...
   WHEN ILAN.DESCRICAO LIKE 'ESTORNO%' THEN '11204001'  
 ...
 CASE WHEN ILAN.MOTIVO_DESCONTO = 'Estorno'   
  ...
  ELSE 'Cobranca: '+CAST(COB.COBRANCA AS VARCHAR)+' - '+CAST(REPLACE(ISNULL(ILAN.DESCRICAO,'Sem Descricao de baixa'),';',',') AS VARCHAR) END AS HISTORICO,  
...
-- '2 - FAT',
 ...
 '01' AS MOEDA,  
 '3' AS TIPO_LANC,  
 CASE WHEN ILAN.DESCRICAO LIKE 'Desconto%' THEN '31401001'  
   WHEN ILAN.DESCRICAO LIKE 'Estorno de Cobrança: Estorno de Bolsa%' THEN '31401003'  
   WHEN ILAN.DESCRICAO LIKE 'Estorno de Cobrança: Estorno de Desconto%' THEN '31401001'  
  ELSE '31401002' END AS CONTA_DEB,  
...
FROM  
 ...
 LEFT OUTER JOIN HADES.dbo.HD_MUNICIPIO AS MUN ON (RESP.END_MUNICIPIO = MUN.MUNICIPIO)  
 LEFT OUTER JOIN FCAV_IMPCONT_CAD AS FCAD ON (ALU.TURMA_PREF = FCAD.TURMA) -- OR T.TURMA = FCAD.TURMA OR HIST.TURMA = FCAD.TURMA)  
 --LEFT OUTER JOIN DADOSADVP12.dbo.VW_FCAV_CT2LANCTO AS VW ON (CAST(ILAN.COBRANCA AS VARCHAR)= VW.COBRANCA)  
WHERE   
 ILAN.DATA >= @DATA_INI  
 AND ILAN.DATA <= (select cast(@data_fim as varchar)+' 23:59:59.000')  
 AND ILAN.DT_ENVIO_CONTAB IS NULL
 AND COB.ESTORNO = 'S' AND ILAN.ITEM_ESTORNADO > 0 
 AND ILAN.DESCRICAO like '%estorno%'  
...
-- '01 - BAIXA',
 ...
 --CASE WHEN ILAN.VALOR LIKE '-%' THEN '2'  
 -- ELSE '1' END AS TIPO_LANC,  
 ...
 --CASE WHEN ILAN.CODIGO_LANC IN ('MS','MT') AND ILAN.CURSO LIKE 'CE%' THEN '31102024'  
 -- ELSE '31102025' END AS CONTA_CRED,  
 CASE 
   WHEN ICRED.DESCRICAO like '%7080-2%' -- Depósito feito em conta corrente e direcionado para a conta contabil correta
  THEN '11102004'
  WHEN ICRED.DESCRICAO like '%04436-7%' -- Depósito feito em conta corrente e direcionado para a conta contabil correta
  THEN '11102013'
  ...
  THEN '11102122'
  WHEN ICRED.DESCRICAO like '%006324-2%' -- Depósito feito em conta corrente e direcionado para a conta contabil correta
  THEN '11102123'
...
FROM  
 ...
 LEFT OUTER JOIN HADES.dbo.HD_MUNICIPIO AS MUN ON (RESP.END_MUNICIPIO = MUN.MUNICIPIO)  
 LEFT OUTER JOIN FCAV_IMPCONT_CAD AS FCAD ON (ALU.TURMA_PREF = FCAD.TURMA) -- OR T.TURMA = FCAD.TURMA OR HIST.TURMA = FCAD.TURMA)  
 --LEFT OUTER JOIN DADOSADVP12.dbo.VW_FCAV_CT2PAGTO AS VW ON (CAST(ICRED.COBRANCA AS VARCHAR)= VW.COBRANCA )  
...
-- '03 - BAIXA', 
 ...
 REPLACE (CAST(((CAST((SELECT SUM(IC.VALOR) FROM LY_ITEM_CRED AS IC WHERE IC.COBRANCA = COB.COBRANCA AND TIPO_ENCARGO IS NULL AND TIPODESCONTO IS NULL) AS NUMERIC(10,2)) * (CAST('0.' AS NUMERIC)+CAST(ISNULL(CTT.CTT_TX_GER,0) AS NUMERIC))) /100) AS NUMERIC(10,2)),'-','')   AS VALOR,  
...
-- *** SELECT EXCLUSIVO PARA GERAR UM REGISTRO A MAIS PARA MONTAR O VALOR DO REPASSE PRO,  
...
-- '06 - BAIXA',  
 ...
 NULL AS DOC,  
 '01' AS FILIAL,  
 (select MAX(IC.ITEMCRED) + 5 from LY_ITEM_CRED as IC where COB.COBRANCA = IC.COBRANCA )AS LINHA,  
 '01' AS MOEDA,  
 '3' AS TIPO_LANC,   
 '41101128' AS CONTA_DEB,  
 '21103007' AS CONTA_CRED,   
 REPLACE (CAST(((CAST((SELECT SUM(IC.VALOR) FROM LY_ITEM_CRED AS IC WHERE IC.COBRANCA = COB.COBRANCA AND TIPO_ENCARGO IS NULL AND TIPODESCONTO IS NULL) AS NUMERIC(10,2)) * 5) /100) AS NUMERIC(10,2)),'-','')   AS VALOR,  
 'Titulo: '+CONVERT(VARCHAR,BOL.BOLETO,20)+' - REPASSE PRO' AS ORIGEM,  
 'Cobranca: '+CAST(COB.COBRANCA AS VARCHAR)+' - REPASSE PRO' AS HISTORICO,  
 FCAD.CENTRO_CUSTO AS CC_DEB,  
 FCAD.CENTRO_CUSTO AS CC_CRED,  
 FCAD.CENTRO_ATIV AS ITEM_CONTAB_DEB,  
 FCAD.CENTRO_ATIV AS ITEM_CONTAB_CRED,  
...
--'8 - BAIXA',
 ...
 CASE WHEN ILAN.CURSO LIKE 'CE%' THEN '31102024' 
	ELSE '31102025' END AS CONTA_DEB,  
 '11204001' AS CONTA_CRED,   
...
SELECT DISTINCT  
 ...
 LOTE,  
 CAST((DENSE_RANK() OVER(PARTITION BY DATA_PROC ORDER BY IMP_BOLETO)) AS VARCHAR (20)) AS DOC,  
