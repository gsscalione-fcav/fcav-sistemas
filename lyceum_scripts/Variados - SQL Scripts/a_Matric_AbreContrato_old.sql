...
CREATE PROCEDURE a_Matric_AbreContrato  --sempre que criar uma tag nova, executar novamente a alteração da procedure.    
(          
 @p_aluno AS T_CODIGO,          
 @p_ano AS T_ANO,          
...
WHERE  
 ...
 WHERE    
  ...
  ---------------------------------------------------------------        
   --TAG para retornar o Tipo de Bolsa   
 INSERT INTO #TAGS_RETORNO (tag, valor)    
   
 SELECT  DISTINCT   
  '%%POSSUI_BOLSA%%',  
  CASE WHEN AL.ALUNO = @p_aluno AND FL_FIELD_22 = AL.CONCURSO AND FL_FIELD_24 IS NOT NULL THEN FL_FIELD_24  
    ELSE  
    '.'  
  END AS POSSUI_BOLSA  
 FROM  
  LY_FL_PESSOA FP  
  INNER JOIN LY_ALUNO AL ON FP.PESSOA = AL.PESSOA  
 WHERE   
  AL.ALUNO = @p_aluno  
  
   ----------------------------------------------------    
   -- TAG para trazer o número mínimo de inscritos    
 INSERT INTO #TAGS_RETORNO (tag, valor)    
 SELECT    
  '%%MINIMO_INSCRITOS%%',    
  ISNULL(CAST(MINIMO_INSCRICOES AS VARCHAR),'') + ' '    
  + '(' + LTRIM(dbo.Numero_Extenso(MINIMO_INSCRICOES)) + ')' AS MINIMO_INSCRITOS    
 FROM    
  LY_OFERTA_CURSO    
 WHERE    
...
ELSE -- TAGS exclusivas para o curso CCOL  
 ...
 INSERT INTO #TAGS_RETORNO (tag, valor)        
  ...
  SELECT DISTINCT  
   ...
   WHEN P.NUM_PARCELAS =  5 then REPLACE(CAST(CONVERT(MONEY,ISNULL((SP.CUSTO_UNITARIO - DP.DESCONTO) / DP.NUM_PARCELAS,0)) AS VARCHAR),'.',',') + ' ' + '(' + ltrim(dbo.Money_Extenso(CONVERT(MONEY,ISNULL((SP.CUSTO_UNITARIO - DP.DESCONTO) / DP.NUM_PARCELAS,
0)))) + ')'   
   WHEN p.NUM_PARCELAS = 10 then REPLACE(CAST(CONVERT(MONEY,ISNULL((SP.CUSTO_UNITARIO - DP.DESCONTO) / DP.NUM_PARCELAS,0)) AS VARCHAR),'.',',') + ' ' + '(' + ltrim(dbo.Money_Extenso(CONVERT(MONEY,ISNULL((SP.CUSTO_UNITARIO - DP.DESCONTO) / DP.NUM_PARCELAS,
0)))) + ')'   
   WHEN p.NUM_PARCELAS = 20 then REPLACE(CAST(CONVERT(MONEY,ISNULL((SP.CUSTO_UNITARIO - DP.DESCONTO) / DP.NUM_PARCELAS,0)) AS VARCHAR),'.',',') + ' ' + '(' + ltrim(dbo.Money_Extenso(CONVERT(MONEY,ISNULL((SP.CUSTO_UNITARIO - DP.DESCONTO) / DP.NUM_PARCELAS,
0)))) + ')'   
   END AS VALOR_MATR    
  FROM     
   LY_LANC_DEBITO L  
