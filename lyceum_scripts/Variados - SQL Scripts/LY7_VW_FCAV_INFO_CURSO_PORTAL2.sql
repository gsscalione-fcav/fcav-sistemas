/*
	VIEW VW_FCAV_INFO_CURSO_PORTAL2
*/
...
SELECT
    ...
    ISNULL((SELECT DISTINCT
        CASE
            WHEN TUR2.UNIDADE_RESPONSAVEL NOT LIKE 'ATUAL' THEN PES2.NOME_COMPL
            ELSE (SELECT
                    NOME_COMPL
                FROM LY_PESSOA AS PES_2
                INNER JOIN LY_COORDENACAO AS FCAV_COORD
                    ON (FCAV_COORD.NUM_FUNC = PES_2.NUM_FUNC)
                WHERE FCAV_COORD.CURSO = TU.DISCIPLINA
                AND FCAV_COORD.TIPO_COORD = 'COORD')
        END
    FROM LY_TURMA AS TUR2
    INNER JOIN LY_CURSO AS CUR2
        ON (TUR2.CURSO = CUR2.CURSO)
    INNER JOIN LY_COORDENACAO AS COOR2
        ON (CUR2.CURSO = COOR2.CURSO)
    INNER JOIN LY_PESSOA AS PES2
        ON (COOR2.NUM_FUNC = PES2.NUM_FUNC)
    WHERE TUR2.TURMA = TU.TURMA
    AND COOR2.TIPO_COORD = 'COORD'), 'NAO CADASTRADO') AS COORDENADOR,
    ISNULL((SELECT DISTINCT
        CASE
            WHEN TUR3.UNIDADE_RESPONSAVEL NOT LIKE 'ATUAL' THEN PES3.NOME_COMPL
            ELSE (SELECT
                    NOME_COMPL
                FROM LY_TURMA TUR_2
                INNER JOIN LY_CURSO AS CUR_2
                    ON (TUR_2.CURSO = CUR_2.CURSO)
                INNER JOIN LY_COORDENACAO AS COOR_2
                    ON (CUR_2.CURSO = COOR_2.CURSO
                    AND TUR_2.TURNO = COOR_2.TURNO
                    AND TUR_2.CURRICULO = COOR_2.CURRICULO)
                INNER JOIN LY_PESSOA AS PES_2
                    ON (COOR_2.NUM_FUNC = PES_2.NUM_FUNC)
                WHERE TUR_2.TURMA = tu.TURMA
                AND COOR3.TIPO_COORD != 'COORD')
        END
    FROM LY_TURMA AS TUR3
    INNER JOIN LY_CURSO AS CUR3
        ON (TUR3.CURSO = CUR3.CURSO)
    INNER JOIN LY_COORDENACAO AS COOR3
        ON (CUR3.CURSO = COOR3.CURSO)
    INNER JOIN LY_PESSOA AS PES3
        ON (COOR3.NUM_FUNC = PES3.NUM_FUNC)
    WHERE TUR3.TURMA = tu.TURMA
    AND COOR3.TIPO_COORD != 'COORD'), '.') AS VICE_COORD,
    tu.CURSO,
    --*** tu.TURNO,  
    tu.CURRICULO,
    ...
    -- *** A REGRA ABAIXO FOI FEITA POR UM PEDIDO DA COMUNICAÇÃO/SECRETARIA PARA UM "AJUSTE" NO LINK DA INSCRIÇÃO SENDO QUE  
    -- *** QUANDO VIER O CCBB T 40, O LINK A SER EXIBIDO SERÁ PARA A TURMA CCGB T 36  
    CASE
        WHEN LO.OFERTA_DE_CURSO = '1142' THEN 'http://189.39.22.133/Inscricao/converte.asp?capac=1141'
        ELSE LINK
    END AS LINK
