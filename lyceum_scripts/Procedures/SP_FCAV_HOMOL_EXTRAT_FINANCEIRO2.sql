/*
    SP utilizada na JOB que alimenta a tabela FCAV_EXTRATO_FINANCEIRO2 WHERE COBRANCA = 21913

	SELECT * FROM FCAV_EXTRATO_FINANCEIRO2 WHERE COBRANCA = 21913
	...
	SELECT ALUNO, VALOR_PAGAR, VALOR_PAGO FROM VW_fcav_EXTFIN_LY WHERE COBRANCA = 21913
...
AS
	...
	BEGIN
		...
		BEGIN
			...
			SELECT DISTINCT -- Há mais de um registro de histórico para o aluno/turma no mesmo dia  
				hist.ALUNO,
				trm.TURMA,
				trm.CURRICULO,
				trm.CENTRO_DE_CUSTO,
			...
			BEGIN
				...
				INSERT INTO #IL_IC
					SELECT
						...
						SUM(VALOR)
					FROM VW_FCAV_IL_NAO_ESTORNO il
					WHERE	NUM_BOLSA IS NOT NULL	AND DESCRICAO NOT LIKE 'Diferença:%'
					GROUP BY COBRANCA
			...
			-- WHERE  (ITEMCOBRANCA <> ITEM_ESTORNADO) OR
			--(MOTIVO_DESCONTO = 'Estorno'	AND ITEM_ESTORNADO IS NULL)
			GROUP BY COBRANCA
		...
		BEGIN
			...
			INSERT INTO FCAV_EXTRATO_FINANCEIRO2_ESTAGE
				SELECT
					...
					--
					ABS(ISNULL(IL_VALOR_LANC_NAO_ACORDADO.VALOR, 0.00) 
					...
					CASE
						...
						WHEN IC_VALOR_PAGO.VALOR IS NOT NULL THEN CASE
								WHEN
									...
									ISNULL(IL_VALOR_LANC_NAO_ACORDADO.VALOR, 0.00)) = 0.00
									--+ ISNULL(IL_BOLSA.VALOR, 0.00) 
									...
									--ISNULL(IC_OUTROS_ENCARGOS.VALOR, 0.00) 
									THEN 'Boleto pago'
						...
						WHEN ISNULL(IL_BOLSA.VALOR, 0.00) < 0.00 AND (ISNULL(IL_VALOR_LANC_NAO_ACORDADO.VALOR, 0.00) +
							ISNULL(IL_BOLSA.VALOR, 0.00) +
							ISNULL(IL_DESCONTO.VALOR, 0.00) +
							ISNULL(IC_DESCONTO.VALOR, 0.00) +
				...
				--  
				LEFT OUTER JOIN VW_COBRANCA_BOLETO cb
					ON cB.COBRANCA = c.COBRANCA
				--  

				LEFT OUTER JOIN LY_BOLETO B
					ON B.BOLETO = cb.BOLETO
				--
				LEFT OUTER JOIN #turma turma
					ON (
					turma.ALUNO = il.ALUNO
					AND turma.CURRICULO = il.CURRICULO
					)
				--  
				LEFT OUTER JOIN #tp_ingresso tp_ingresso
					ON (
					tp_ingresso.ALUNO = il.ALUNO
