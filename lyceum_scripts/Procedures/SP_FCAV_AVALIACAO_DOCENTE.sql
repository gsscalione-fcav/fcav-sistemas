/*
	...
	SELECT * FROM #tmp_respostas_alunos WHERE APLICACAO LIKE '%DDBDON%'
	SELECT * FROM #tmp_avaliacao_disciplinas  WHERE APLICACAO LIKE '%DDBDON%'
	SELECT * FROM #tmp_alunos_turma_disicplina WHERE TURMA LIKE 'D-DBDON%' AND DISCIPLINA LIKE 'CEAI%TCC%'
...
AS
	...
	BEGIN
		...
		SELECT 
			...
			AL.SEMESTRE,
			CASE WHEN CS.CURSO = 'CEAI' THEN 
					REPLACE(REPLACE(REPLACE(AC.TURMA_CEAI,' ',''),'/',''),'-','') 
