...
ALTER PROCEDURE OBTER_DADOS_BOLETO_REGISTRO  
(  
 -- DADOS COMUNS  
...
-- [INÍCIO]   
 ...
 DECLARE @v_valor_multa T_DECIMAL_MEDIO   
    DECLARE @v_1_Instrucao VARCHAR(2)   
 DECLARE @v_2_Instrucao VARCHAR(2)   
 ...
 BEGIN  
  ...
  BEGIN -- OBTEM DADOS DO BOLETO DE ALUNO  
   ...
   SELECT @v_cpf_cnpj_pagador = rf.CPF_TITULAR,  
...
--   INNER JOIN LY_PESSOA p ON p.PESSOA = rf.PESSOA  
  ...
  BEGIN -- OBTEM DADOS DO BOLETO DE CANDIDATO  
   ...
   BEGIN  
    ...
    -- OBTEM INFORMAÇÕES DO PAGADOR  
    SELECT @v_cpf_cnpj_pagador = c.CPF,  
   ...
   BEGIN  
    ...
    SELECT @v_cpf_cnpj_pagador = c.CPF,  
        ...
        @v_complemento_pagador = c.END_COMPL  
    FROM LY_CANDIDATO c  
    INNER JOIN LY_BOLETOS_CANCELADOS bc ON c.CANDIDATO = bc.CANDIDATO  
             AND c.CONCURSO = bc.CONCURSO  
    LEFT JOIN HD_MUNICIPIO m ON m.MUNICIPIO = c.MUNICIPIO  
    WHERE c.BOLETO = @p_Boleto  
 ...
 EXEC DIVIDE_INSTRUCOES_BOLETO @p_Boleto, @p_Boleto_candidato, @v_instrucao_linha_1_boleto,   
 ...
 SET @p_mensagem_servico_rest = '  
  {  
   ...
   "eh_Producao": "'+ISNULL(CONVERT(VARCHAR(100), @p_ehProducao), '')+'",  
      ...
      "descricao_compra_controle": "'+ISNULL(CONVERT(VARCHAR(100), @v_descricao_compra_controle), '')+'",  
   ...
   "cod_convenio_beneficiario": "'+ISNULL(CONVERT(VARCHAR(100), @v_cod_convenio_beneficiario), '')+'",  
    ...
    "pagador_Parametro": {  
      ...
      "nome_pagador": "'+ISNULL(CONVERT(VARCHAR(100), @v_nome_pagador), '')+'",  
      "logradouro_pagador": "'+ISNULL(CONVERT(VARCHAR(100), @v_logradouro_pagador), '')+'",  
      "bairro_pagador": "'+ISNULL(CONVERT(VARCHAR(100), @v_bairro_pagador), '')+'",  
      "cidade_pagador": "'+ISNULL(CONVERT(VARCHAR(100), @v_cidade_pagador), '')+'",  
      "nome_municipio_pagador": "'+ISNULL(CONVERT(VARCHAR(100), @v_nome_municipio_pagador), '')+'",  
      "numero_endereco_pagador": "'+ISNULL(CONVERT(VARCHAR(100), @v_numero_endereco_pagador), '')+'",  
      "complemento_pagador": "'+ISNULL(CONVERT(VARCHAR(100), @v_complemento_pagador), '')+'",  
   ...
   "tipo_documento_sacador_avalista": "'+ISNULL(CONVERT(VARCHAR(100), @v_tipo_documento_sacador_avalista), '')+'"  
    ...
    IF @v_percentual_juros IS NOT NULL or @v_valor_juros IS NOT NULL  
    BEGIN  
  ...
  END  
   ...
   --  SET  @v_perde_bolsa_valor =REPLACE(REPLACE(@v_perde_bolsa_valor, CHAR(13), ' '), CHAR(10), ' ');  
    ...
    SET @p_mensagem_servico_rest = @p_mensagem_servico_rest + '"grupo_Desconto_Parametro": [';  
  ...
  BEGIN  
   ...
   BEGIN  
    ...
    SELECT DISTINCT dc.TIPO_DESC, ISNULL(dc.DT_VENC_DESC,@v_data_vencimento_boleto), CONVERT(VARCHAR, SUM(dc.VALOR)) FROM LY_DESCONTO_COBRANCA dc  
