...
SELECT 
	...
	CONVERT(DECIMAL(10,2),((CAST(SUM(BOM)AS decimal(10,2))+SUM(EXCELENTE))/ CASE WHEN SUM(QTDE_PREENCHIDA)= 0 THEN  1 ELSE SUM(QTDE_PREENCHIDA) END )) AS INDICE_APROVACAO,
	CONVERT(DECIMAL(10,2),((CAST(SUM(REGULAR)AS decimal(10,2))+SUM(RUIM)+SUM(PESSIMO))/CASE WHEN SUM(QTDE_PREENCHIDA)= 0 THEN  1 ELSE SUM(QTDE_PREENCHIDA) END)) AS INDICE_REJEICAO
