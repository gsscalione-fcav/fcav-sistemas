/*
	EXEC PR_FCAV_LYCEUM_OFERTA_CURSO_ATUALIZA_PORTAL 12603,'S'

	EXEC PR_FCAV_LYCEUM_OFERTA_CURSO_EMAIL_COMUNICACAO 12604,'S'

	select DISCIPLINA, TURMA, DT_INICIO, CLASSIFICACAO  from ly_turma where TURMA IN ('A-FAI9001 T 32') order by DT_INICIO

	SELECT DISTINCT OFERTA_DE_CURSO, CURSO,TURNO, CURRICULO, TURMA, SERVICO_VINCULADO, CUSTO_UNITARIO FROM VW_FCAV_CONFERENCIA_DE_CADASTRADOS_TURMAS WHERE TURMA = 'CEGP T 77'
	SELECT * FROM ly_oferta_curso where oferta_de_curso = 12422
	SELECT * FROM ly_oferta_curso where concurso = 'CEAI T 39'
	SELECT * FROM ly_opcoes_oferta where oferta_de_curso = 12422
	SELECT * FROM ly_curriculo WHERE curso = 'A-DT'
	SELECT * FROM ly_grade WHERE curso = 'A-DT'
