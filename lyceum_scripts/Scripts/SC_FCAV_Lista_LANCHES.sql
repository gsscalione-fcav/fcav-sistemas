...
CREATE VIEW VW_FCAV_LISTA_LANCHES
AS
...
SELECT 
	...
	COUNT(ALUNO) ALUNOS,
	COUNT(ALUNO) + 1 +
	CASE WHEN EXISTS(SELECT 1 FROM VW_FCAV_MONITOR_AULA MO WHERE MO.DISCIPLINA = AG.DISCIPLINA AND MO.DATA = AG.DATA
		)THEN 1 ELSE 0 END AS LANCHES,
	CANCELADA	
FROM
